  ...
  return (
    ...
    >
      <div className="space-y-4">
        <div className="px-3 flex items-center gap-2 text-white/95 font-bold text-xs">
          <div className="w-6 h-6 rounded-md bg-blue-600 flex items-center justify-center shrink-0 shadow-sm">
        ...
        <div className="space-y-1 px-2">
          {items.map((item, idx) => (
            <div
              ...
              style={item.active ? { backgroundColor: `${theme.palette.accent}30`, borderLeft: `3px solid ${theme.palette.accent}` } : {}}
            >
      ...
      <div className="px-3 pt-2 border-t border-white/10 flex items-center justify-between text-slate-400 text-[10px]">
        <span className="hidden md:inline">2026.9.2</span>
        <Power className="w-3.5 h-3.5 text-red-400 hover:text-red-300 cursor-pointer" />
      </div>
    </div>
  );
};
